...
const ImportPage = () => {
  ...
  const editPhieu = async (phieu: any) => {
    try {
      const res = await api.get(`/import/${phieu.id}`);
      const data = res.data;
      setEditingId(phieu.id);
      setNhaCungCap(data.phieu_nhap.nha_cung_cap || '');
      if (Array.isArray(data.chi_tiet) && data.chi_tiet.length > 0) {
        // Map dữ liệu chi tiết từ API sang state chiTiet
        setChiTiet(
      ...
      console.error(err);
      alert('Không thể lấy dữ liệu phiếu nhập để sửa!');
    }
  };
  ...
  const deletePhieu = async (id: number) => {
    ...
    try {
      ...
      console.error(err);
      alert(err.response?.data?.detail || 'Lỗi khi xóa phiếu!');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!nhaCungCap.trim()) return alert('Vui lòng nhập tên nhà cung cấp!');
    if (chiTiet.some(item => item.product_id === 0)) return alert('Vui lòng chọn sản phẩm!');
    if (chiTiet.some(item => item.so_luong <= 0 || item.gia_nhap < 0))
      return alert('Số lượng và giá nhập phải hợp lệ!');
    ...
    try {
      ...
      console.error(err);
      alert(err.response?.data?.detail || 'Lỗi khi lưu phiếu!');
    }
  };
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(i => (
                <tr key={i.id}>
                  ...
                  <td>{Number(i.tong_tien || 0).toLocaleString()} ₫</td>
                  <td style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                    <button onClick={() => editPhieu(i)} style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}>
                      <Edit2 size={18} color="#4caf50" />
                    </button>
                    <button onClick={() => deletePhieu(i.id)} style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}>
                      <Trash2 size={18} color="#f44336" />
                    </button>
                    <button onClick={() => xemChiTiet(i.id)} style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}>
                ...
                {chiTiet.map((item, index) => (
                  <div key={index} style={{ display: 'flex', gap: '10px', marginBottom: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
                    <select
                      ...
                      onChange={e => {
                        const newArr = [...chiTiet];
                        newArr[index].product_id = Number(e.target.value);
                        setChiTiet(newArr);
                      }}
                      required
                    ...
                    <input
                      ...
                      onChange={e => {
                        const newArr = [...chiTiet];
                        newArr[index].so_luong = Number(e.target.value) || 1;
                        setChiTiet(newArr);
                      }}
                      style={{ width: '100px' }}
                      required
                    ...
                    <input
                      ...
                      onChange={e => {
                        const newArr = [...chiTiet];
                        newArr[index].gia_nhap = Number(e.target.value) || 0;
                        setChiTiet(newArr);
                      }}
                      style={{ width: '130px' }}
                      required
                    ...
                    <span style={{ width: '100px', fontWeight: 'bold' }}>
                      {(item.so_luong * item.gia_nhap).toLocaleString()} ₫
                    </span>
                    {chiTiet.length > 1 && (
                      <button type="button" className="btn-delete" onClick={() => xoaDong(index)}>Xóa</button>
                    )}
                  </div>
                ))}
                <button type="button" className="btn-primary" onClick={themDong}>+ Thêm sản phẩm</button>
      ...
      {showDetailModal && selectedPhieu && (
        <div className="modal-overlay" onClick={() => setShowDetailModal(false)}>
          <div className="modal-content" style={{ maxWidth: '800px' }} onClick={e => e.stopPropagation()}>
            <h3>Chi tiết phiếu nhập: {selectedPhieu.phieu_nhap.ma_phieu}</h3>
            <div className="table-container" style={{ marginTop: '20px' }}>
              <table>
                ...
                <tbody>
                  {selectedPhieu.chi_tiet.map((item: any, index: number) => (
                    <tr key={index}>
                      <td>{item.ten_sp}</td>
                      <td>{item.ma_sp}</td>
                      <td>{item.so_luong}</td>
            ...
            <div style={{ marginTop: '15px', textAlign: 'right', fontWeight: 'bold' }}>
              Tổng tiền phiếu:{' '}
              {selectedPhieu.chi_tiet
                .reduce((sum: number, item: any) => sum + item.so_luong * item.gia_nhap, 0)
                .toLocaleString()} ₫
            </div>
